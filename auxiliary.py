...
def Plot_Features(p: int, M_: int, number_bins: int, min_N_: int, max_N_: int, number_N_: int, True_Features: np.array, f_type: int):
    ...
    plt.title('Feature selection with bins number = ' + str(number_bins) +  
              ', number of all features = ' + str(p) + ', number of relevant features = ' + str(2 * M_))
    
    plt.xlabel('Sample size')
    plt.ylabel('Positive Selection Rate')

    plt.ylim(0, 1.1)

    plt.plot(N, Succes_Rate, marker='x', label='f_type = ' + str(f_type)) #, label='Market SSR')
    plt.legend()
    plt.show()
